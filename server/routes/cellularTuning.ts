...
export = function cellularTuningRoutes ({ authenticateToken, toBool, cellularTuning }: { authenticateToken: any; toBool: any; cellularTuning: any }) {
  const router = Router()

  // cellular video tuning settings and status
  router.get('/api/cellulartuning', authenticateToken, (req: Request, res: Response) => {
