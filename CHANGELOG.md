### Changelog
#### Unreleased (fork)
 - UI: **Layout & contrast polish pass, guided by [impeccable](https://github.com/pbakaus/impeccable).** Installed impeccable (a design skill + a deterministic anti-pattern detector + an opt-in edit-time hook) into the gitignored `.claude/`, seeded `PRODUCT.md` + `DESIGN.md` (capturing the existing Ground Station identity so the tooling is identity-preserving) and an `.impeccable/config.json` that ignores the two deliberate brand "tells" (the amber side-tab accent; the reduced-motion-gated sidebar/nav transitions). Then fixed what the detector surfaced. **Root cause of the cramped look:** `styles.css` only ever styled the page-title `h1` — in-page section headings (`h2`/`h3`/`h4`) had *no* margin, so every section sat flush against the content above it, and pages papered over it with inconsistent inline hacks. Now a **spacing-scale token set (`--gs-space-1..7`)** drives real **section-heading rhythm** (`.pagedetails h2/h3/h4`: deliberate top/bottom margins, an `h2` underline, uppercase `h2` matching the instrument-label system) and **form-row rhythm** (`.form-group` ≈12 px gap), so the ad-hoc `marginBottom:'5px'`/`'0px'` (≈invisible) form rows, `marginTop:'20px'` heading hacks and `8px` card gaps were deleted across **all 19 fork config pages** (`networkconfig`/`ltemodem`/`flightcontroller`/`video`/`home` first, then `adhocwifi`/`cameraswitcher`/`cellulartuning`/`cloud`/`ddns`/`logBrowser`/`ntripcontroller`/`pipelineeditor`/`ppp`/`secondarystreams`/`tailscale`/`telemetryinjector`/`vpnconfig`/`wireguardhub`). **Accessibility:** `--gs-text-faint` was below WCAG AA on both themes (light 2.9:1, dark 3.6:1 in inputs) — darkened/brightened to ≥4.5:1 (light `#5d6b7a`, dark `#7b8ea6`), fixing placeholder/footer/disabled text; the over-wide `0.2em` brand-sub tracking was eased to `0.12em`. **Theme bug:** the LTE AT-console `<pre>` had a hard-coded `#f8f9fa` background (a white box in dark mode) — now inherits the themed `pre`. **Hierarchy:** Home's page section used `<h5>` (an `h1→h5` skip) — now `<h2>`. Rendered-detector findings on Home dropped 23→14 (the real low-contrast + the layout-transition noise gone; the residual 14 are documented non-issues: 12 false-positives from the opaque cards' decorative sheen gradient, 1 intentional eyebrow-label tracking, 1 minor card-title level). CSS + markup only — no logic touched; both suites stay 100/100/100/100, lint clean. See DESIGN.md / PRODUCT.md.
 - Feature (#173): **Selectable artificial-horizon style, aircraft marker + colours (INAV-inspired).** The artificial horizon was one fixed style with a hard-coded yellow aircraft symbol. It's now configurable from the editor's horizon panel, mirroring INAV's `osd_ahi_style` + `osd_crosshairs_style` ([iNavFlight/inav](https://github.com/iNavFlight/inav)): a **Style** (pitch *ladder* · *line* only · centre *ticks*), an **Aircraft** centre marker (*wings* · *crosshair* · *dot* · *caret* · *drone*), and independent **colours** for the horizon lines and the aircraft symbol (the compass and home arrow also take a colour). `hudOverlay.ts` validates the choices (new `horizonOptions()`, exposed via `GET /api/hudlayout`); `video-server.py`'s `_horizon_svg`/`_marker_svg` render them; the editor mini-preview reflects them live. Defaults unchanged (ladder + wings, green/amber). Both suites stay 100/100/100/100; all 15 marker×style combos verified through a real `rsvgoverlay` pipeline. See docs/reports/feature-32-hud-overlay.md.
 - Feature (#173): **Resizable HUD graphic elements (artificial horizon / compass / home arrow).** The graphic OSD elements were fixed-size — small and not adjustable, since the *Size* control only affected text fields. They now carry a **`scale` multiplier (0.5–5×)** with a **Size slider** in the editor's per-element panel, applied in both the editor preview and the burned-in HUD (`video-server.py` wraps each graphic in a `scale()` group about its centre). Defaults are bumped so they're prominent out of the box (horizon **1.8×**, compass/home arrow **1.4×**) — so existing layouts get a bigger horizon automatically. `hudOverlay.ts` validates/clamps the scale; both suites stay 100/100/100/100; the scaled horizon verified through a real `rsvgoverlay` pipeline. See docs/reports/feature-32-hud-overlay.md.
 - Feature (#173): **HUD editor — live camera feed behind the OSD + two-column element list.** The editor can now show a **live MJPEG feed of a camera as the canvas backdrop**, so you position the HUD over the real picture: a *Show camera feed behind the HUD* toggle + a camera **select box** (populated from `/api/videodevices`). A new `GET /api/camera/preview` route streams `multipart/x-mixed-replace` JPEG frames from `python/mjpeg-preview.py` (a self-contained GStreamer pipeline reusing the same source handling as `video-server.py` — libcamera for CSI, v4l2 for USB), which the browser renders in an `<img>`. Auth now also accepts a **`?token=` query param** (an `<img>`/`EventSource` can't set headers). It only runs when the camera is **idle** (a single CSI device can't be opened twice — the preview is stopped when the main stream starts, and 409s if it's already streaming); pre-compressed H264/H265 sources aren't previewable (same raw-video constraint as the burned-in HUD). Separately, the long telemetry-element palette is now laid out in **two columns** (grouped by section, balanced) to cut scrolling. Both suites stay 100/100/100/100; the MJPEG plumbing verified on real GStreamer (179 frames, correct multipart boundary). See docs/reports/feature-32-hud-overlay.md.
 - Fix (modem/QMI): **`qmicli` now uses the qmi-proxy** (`--device-open-proxy`) for both `_qmiConnect` and `_qmiStop`. Without it, `qmicli` opens the `cdc-wdm` device exclusively, so a call can fail with a *CID-allocation timeout* when the kernel/`qmi_wwan` or another call is also using the device; and the `--client-no-release-cid` kept at connect only survives to the matching `--wds-stop-network` when both invocations share the proxy. Found while bringing up a SIM7600 in QMI mode on-device (a plain `qmicli` query timed out where the proxy version worked). Both suites stay 100/100/100/100. See docs/MODEM-DATA-PATH.md.
 - Feature (#173): **LTE modem GNSS surfaced on the Modem page + during discovery.** The SIM7600's own satellite fix (already polled for the video HUD's *Modem GPS* elements) now also shows on the **LTE Modem** page: a **GNSS (modem GPS)** status row renders the live fix (lat/lon, altitude, an OpenStreetMap link) when there's a lock, "No fix · acquiring…" while the modem is up without one, or "—" when the modem isn't responding. The **modem discovery scan** also previews GNSS — `probeAttempt` now asks each AT responder for its fix (`AT+CGPS=1` + `AT+CGPSINFO`), so the scan result shows a **GNSS fix** badge with the position, letting you confirm the GPS antenna works *before* enabling the monitor (handy when the AT port isn't the default `ttyUSB2` — on a SIM7600 in QMI/`9001` mode it's often `ttyUSB3`). `parseCGPSINFO` now tolerates a no-response (null) reading. Both suites stay 100/100/100/100. See docs/reports/feature-32-hud-overlay.md.
 - Feature (#173): **Custom HUD fonts — curated faces (incl. a DJI/FPV-OSD-style font) + user import.** The OSD font picker grew from the 3 generic families to a real font system. A **curated** set ships bundled: **Oxanium** (a DJI/FPV-OSD-style display face), **Chakra Petch**, **IBM Plex Mono** and **IBM Plex Sans** (all OFL, in `assets/hudfonts/`). Users can also **import** their own `.ttf`/`.otf` from the HUD Editor (an upload control + a removable list of imports). The hard part is WYSIWYG: the editor preview is a *browser* font but the burned-in HUD is rendered on the device by `rsvgoverlay → librsvg → Pango → fontconfig`, so a new `server/hudFonts.ts` keeps **one** fonts directory that is both **served to the browser** (`GET /api/hudfonts/file/:name`, injected as `@font-face` so the preview is exact) and made visible to the spawned `video-server.py` via **`XDG_DATA_HOME`** (fontconfig scans `$XDG_DATA_HOME/fonts`) — **no sudo, no system dirs, no service-user assumptions**. Import validates the upload (sfnt magic, ≤5 MB), installs it (`fc-cache`), reads its family (`fc-query`) and registers it; the manager copies the curated `.ttf` in on startup. Routes: `GET/POST/DELETE /api/hudfonts` + the file route; `hudOverlay.ts` now accepts any safe family name (letters/digits/space/hyphen). Packaging ships `assets/` in the `.deb` and adds a `fontconfig` dependency; the global upload limit was raised from 1 KB to 6 MB (every upload route stays authenticated + validated). Both suites stay 100/100/100/100; the curated + a styled SVG render verified through a real `rsvgoverlay` pipeline on WSL and on the Pi. See docs/reports/feature-32-hud-overlay.md.
 - Fix (tests): **`covfront` now exits 0** — `serviceWorker.test.js`'s localhost test fired `checkValidServiceWorker` → `fetch(swUrl)` with `fetch` unstubbed and un-awaited; that dangling promise could resolve to a 404/non-JS body during a *later* test and run `navigator.serviceWorker.ready.then(r => r.unregister().then(…))` against that test's redefined service worker (whose `unregister()` returns `undefined`), surfacing as an unhandled rejection at `serviceWorker.js:105` and failing the run (exit 1) even though all 858 tests passed at 100% coverage. The test now stubs `fetch` to the deterministic offline branch and awaits a settle so nothing dangles. Test-only; both suites stay 100/100/100/100.
 - Feature (#173): **HUD editor — modem GPS, WYSIWYG mock data, dynamic home arrow, per-element & global text styling.** Four upgrades to the OSD editor: (1) **LTE modem GPS** — the SIM7600's own GNSS fix is now an overlay source (a new *Modem GPS* section: fix state, lat, lon, alt), available **even with no flight controller connected**. `ltemodem.ts` enables the GNSS once (`AT+CGPS=1`) and parses `AT+CGPSINFO` each poll (new `parseCGPSINFO`, ddmm.mmmmmm→deg); `videostream.ts` folds the fix into the HUD (`mergeModemGps`) and runs a **periodic 1 Hz graphic-HUD push** so modem GPS renders without MAVLink traffic. (2) **Mock data on the editor canvas** — each chip now shows the value exactly as it will be burned onto the video (e.g. `ALT 124m`, `mGPS OK`) instead of a placeholder label (WYSIWYG); the catalog carries a `mock` string per element. (3) **Dynamic home arrow** — the home-direction element renders as a live arrow that rotates (the burned-in HUD already rotates it by the real bearing-to-home; the editor shows a slow spin to convey it tracks home like a compass). (4) **Font / size / colour** — every text field's font family, size (10–120 HUD units) and colour are editable, with a **global** default style and **per-element overrides** (blank = inherit). `hudOverlay.ts` validates/normalises the global + per-element style, `video-server.py`'s `buildHudSvg` applies them, and the editor gains a global-style row + a per-element style panel (`hudFonts()` / `GET /api/hudlayout` now also returns the font list). Both suites stay 100/100/100/100; the styled SVG + modem GPS render verified through a real `rsvgoverlay` pipeline. See docs/reports/feature-32-hud-overlay.md.
 - Feature (#173): **Full OSD element set, grouped in sections** — the HUD editor's catalog grew from 14 to ~45 telemetry elements organised into sections (Attitude, Altitude & Speed, Position & GPS, Navigation, Battery & Power, Link, Environment, Status, Health). New elements: a graphic **compass tape** and **home-direction arrow**, turn rate, G-load, rangefinder, lat/lon, GPS HDOP/course, **distance & bearing to home** (computed from `HOME_POSITION`), distance/number to next waypoint, crosstrack/altitude error, mAh consumed, battery temp & time-remaining, autopilot load, RC RSSI, radio RSSI/remote/noise, comm drop rate, wind speed/direction, baro temp/pressure, vibration & clipping, a **flight timer** (since arm) and a clock. `videostream.ts`'s tap now reads `BATTERY_STATUS`, `NAV_CONTROLLER_OUTPUT`, `MISSION_CURRENT`, `RC_CHANNELS`, `RADIO_STATUS`, `WIND`, `SCALED_PRESSURE`, `RANGEFINDER`, `VIBRATION`, `SCALED_IMU` and `HOME_POSITION`; the editor palette is grouped by section; `video-server.py` renders all of them. Both suites stay 100/100/100/100. See docs/reports/feature-32-hud-overlay.md.
 - Feature (#173): **Customizable HUD / OSD editor** — the graphic HUD is now fully configurable from a new **HUD Editor** page (Camera & Video nav), iNav-style: a black 16:9 canvas where you **drag** each telemetry element where you want it, plus a palette to toggle which stats show and whether each draws its **icon**. The artificial horizon is itself a toggleable, positionable element. The expanded element set covers altitude (MSL/AGL), ground speed, airspeed, heading, climb rate, throttle, battery V/%/current, flight mode, arm state and GPS — `videostream.ts`'s telemetry tap now captures all of these (adding `GLOBAL_POSITION_INT`, plus airspeed/climb/throttle/current/armed). The layout is a list of `{type,enabled,x,y,icon}` (x/y as 0–1 frame fractions) saved in settings and pushed to the video server over the existing stdin channel (`{cmd:'hudlayout',…}`); `video-server.py`'s `buildHudSvg` became **layout-driven**, placing each enabled element's icon glyph + value (and the horizon) at its position via the SVG/`rsvgoverlay` renderer (no new dependency). New `server/routes/hud.ts` (`GET`/`POST /api/hudlayout` + the element catalog) and `src/hudeditor.jsx`. The simple **Text** HUD style is unchanged. Both suites stay 100/100/100/100. See docs/reports/feature-32-hud-overlay.md.
 - Feature (#398 follow-up): **Multi-stream MAVLink discovery** — the camera protocol now advertises the **secondary streams** too, not just the primary. `videostream.ts`'s `VIDEO_STREAM_INFORMATION` response was generalised from a single hard-coded stream to a list (primary + each secondary), so a GCS that discovers streams over MAVLink (e.g. QGroundControl) sees the correct `count` and one entry per stream, each with its own URI (secondary RTSP → `rtsp://<ip>:8555+slot/<mount>`, secondary RTP → its UDP port), encoding, resolution, fps and bitrate. A request for a specific `streamId` returns just that stream; `streamId 0` returns all. The video manager is handed the secondary-stream manager from `index.ts` for this. Both suites stay 100/100/100/100. See docs/reports/feature-34-secondary-streams.md.
 - Feature (#173 follow-up): **Graphic artificial-horizon HUD** — the Telemetry HUD now has a **style** choice on the Video page: *Text readout* (the original corner text) or *Graphic* (an artificial-horizon overlay — attitude horizon + pitch ladder + corner readouts). Implemented with **no new dependency**: `video-server.py` renders the HUD as an **SVG via `rsvgoverlay`** (present on the Pi's GStreamer) instead of `textoverlay`, fed live over the existing stdin control channel; `videostream.ts` now also captures `ATTITUDE` (roll/pitch) and, in graphic mode, pushes the raw fields (the SVG is built in the video server). Selectable per the same raw-video constraint as the text HUD. Both suites stay 100/100/100/100. See docs/reports/feature-32-hud-overlay.md.
