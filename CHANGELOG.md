### Changelog
#### Unreleased (fork)
 - Refactor: Modularize the two largest backend files (behaviour-preserving, gated by full CI parity; both suites stay 100/100/100/100). `server/index.js` 2196→1215 lines (−45%): 14 cohesive route groups extracted into `server/routes/*.js` router-factory modules (DI context object) — ltemodem, cameraSwitcher, customPipelines, cellularTuning, telemetryInjector, networkPriority, dynamicDns, ppp, vpn, ntrip, cloud, logConversion, adhoc, network. `server/videostream.js` 1099→1031: pure helpers (path/codec/transport/interface/mavchar) moved to `server/videostreamHelpers.js`, the class delegates (public API unchanged). See docs/reports/split-index-videostream-2026-06.md.
